...
% Runge-Kutta methods
% Cut-off function with jump discontinuity
...
dname=strcat(dpath,"iontrap_jump.mat");

% Set the figure path
fpath=strcat(mpath,'fig/');
fname=strcat(fpath,'iontrap_jump.eps');
...
if ~isfile(dname)
    ...
    rho=0.1462/2; rho1=[]; rho2=[]; 
    
    % Set cut-off a function with jump discontinuity
    cut=@(x)trans0(rho,x);
