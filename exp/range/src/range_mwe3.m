...
dname=strcat(dpath,"range_mwe3.mat");

% Set the figure path
fpath=strcat(mpath,'fig/');
fname(1)=strcat(fpath,'range_mwe3.eps');
...
if ~isfile(dname)
    ...
    % Define an event function that respects the boundary at 11,000 meters
    g=@(x)x(2)*(x(2)-11000);

    % Track 3 events! Pass the boundary twice and hit the ground once.
    K=3;
